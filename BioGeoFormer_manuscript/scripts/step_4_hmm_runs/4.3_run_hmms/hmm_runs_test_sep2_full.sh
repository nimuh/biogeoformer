...
test_fasta_dir="/nfs5/MICRO/Thurber_Lab/jacob/hmm_runs_sep2/test_fasta_aug26"
output_dir="/nfs5/MICRO/Thurber_Lab/jacob/hmm_runs_sep2/test_output_aug26_full"
...
for hmm_model in ${hmm_model_dir}/selected_train_*.hmm; do
    ...
    for fasta_file in ${test_fasta_dir}/final_selected_test_${similarity}.fasta; do
        if [[ -f $fasta_file ]]; then
            model_base=$(basename "$hmm_model" .hmm)
            output_file="${output_dir}/${model_base}_on_test_${similarity}.tblout"
