...
folder_path <- "../../hmm_run_val_test_anti_split/test_output_hmm/"
...
str(filtered_unique_hmm_data_split)
...
csv_data_list <- lapply(csv_data_list, function(df) {
  if ("sequence" %in% names(df)) {
    # Trim spaces/newlines like Python would keep literal chars
    df$sequence <- gsub("\\s+", "", df$sequence)
    df[nchar(df$sequence, type = "chars") <= 1024, ]
  } else {
    df
  }
})
...
merged_data_list <- lapply(merged_data_list, function(df) {
  if ("sequence" %in% names(df)) {
    # 1. Remove whitespace/newlines so counts match Python's len()
    df$sequence <- gsub("\\s+", "", df$sequence)
    
    # 2. Keep only sequences ≤ 1024 characters
    df <- df[nchar(df$sequence, type = "chars") <= 1024, ]
  }
  df
})
...
output_dir <- "../../hmm_run_val_test_anti_split/test_processed_hmm/"
